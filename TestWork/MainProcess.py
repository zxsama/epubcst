...
def epub_read(epub_out_file_path_sour, chunk_size=32*32):
    ''' 
    参数：已读取的压缩包，文件块大小
    读取文件
    Lazy function (generator) to read a file piece by piece.
...
def file_translate(file_convert_read):
    '''
    参数：文件块，翻译方式
    繁简翻译 
    '''
...
def epub_file_deal(file_path):
    ...
    for epub_file_path_r in epub_list_r:
        epub_pool.apply_async(epub_run_process, args=(epub_file_path_r,))
    epub_pool.close()
    epub_pool.join()
...
def epub_run_process(epub_file_path_r):
    '''
    并发任务  
    '''
    ...
    e_staf == "XHTML" or e_staf == "OPF" or e_staf == "txt" or e_staf == "TXT"):
        ...
        for chunk in epub_read(epub_out_file_path_sour):
            file_converted = file_translate(chunk)
...
def file_type_check(file_info, file_path):
    ''' 
...
if __name__ == "__main__":
    ...
    file_type_check(file_info, file_path)
