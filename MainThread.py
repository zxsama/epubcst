...
def epub_file_deal(file_path, cc_st):
    ...
    for e_lsit in epub_namelist:
        epub_list_r.append([e_lsit, epub_r, epub_extr_path])
    epub_pool = ThreadPool(processes=20)
    epub_pool.map(epub_run_process,(epub_file_path_r for epub_file_path_r in epub_list_r))
    epub_pool.close()
...
def epub_run_process(epub_file_path_r):
    ...
    epub_r = epub_file_path_r[1]
    epub_extr_path = epub_file_path_r[2]
    try:
